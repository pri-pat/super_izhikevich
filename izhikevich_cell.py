# -*- coding: utf-8 -*-
"""
Created on Wed Jul 14 12:50:14 2021
...
def plotMyData(somecell, upLim = 200):
    tau = somecell.tau
    n = somecell.n
    v = somecell.v
...
class izhCell():
    def __init__(self, stimVal):
        self.celltype = 'Regular Bursting'
        self.a = 0.03
        self.b = -2
        self.b = -50
        self.d = 100
        ...
        #Results
        self.v = self.vr = np.ones()
        self.u = 0*self.v
    
    def __repr__(self):
        return self.celltype + ' Cell with Input Current = ' + str(self.stimVal)
    ...
    def stimulate(self):
        for i in range(0,self.n-1):
            ...
            if self.v[0,i+1]>=self.vpeak:
                self.v[0,i]=self.vpeak
                self.v[0,i+1] =self.cself.u[0,i+1] =self.u[0,i+1] +self.d
   
    def plotMyData(somecall,upLim=200):
        tau = somecell.tau
        n=somecell.n
        celltype = somecell.celltype
        
        fig=plt.figure()
        plt.plot(tau*np.arrange(0,n),v[0,:].transpose(),'k-')
        plt.xlabel('Time Step')
        plt.xlim([0,upLim])
        plt.ylabel(celltype + 'Cell Response')
